...
const SupplierMasterData = () => {
  ...
  useEffect(() => {
    if (supplierMasterData) {
      const { transformedColumns, transformedRows } = transformData(supplierMasterData);
  ...
  const transformData = (data) => {
    ...
    const keys = Object.keys(data[0]).filter(key => 
      key !== 'on_time_delivery_score' && key !== 'quality_score'
    );
    ...
    const transformedRows = data.map((item, index) => {
      const { on_time_delivery_score, quality_score, ...rest } = item;
      return {
        id: index + 1,
        ...rest,
      };
  ...
  const handleAddRow = async () => {
    try {
      const response = await axios.post(`${MASTER_DATA }/supplierMasterData/createSupplier`, newRow);
  ...
  const handleDeleteRow = async (row) => {
    try {
      await axios.delete(`${MASTER_DATA }/supplierMasterData/deleteSupplier/${row.supplier_id}`);
  ...
  const handleSaveEditedRow = async () => {
    try {
      const response = await axios.put(`${MASTER_DATA }/supplierMasterData/updateSupplier/${editedRow.supplier_id}`, editedRow);
            ...
            <FormControl fullWidth key={key} style={{ marginBottom: '16px' }}>
              {key === 'minority_supplier' ? (
                ...
                <TextField
                  ...
                  value={newRow[key]}
                  onChange={handleNewRowChange}
                  type={['zip_code', 'financial_score', 'sustainability_score'].includes(key) ? 'number' : 'text'}
